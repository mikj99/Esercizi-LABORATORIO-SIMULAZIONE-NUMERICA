...
int main() { 
	...
	cout << "\nPrima della simulazione scegliere quake fase simulare [solid/liquid/gas]:" <<endl;
	cin >> state;
...
//Equilibrazione + condizione iniziale
	for (int i=0; i<2000; i++)
		Move();
	Measure();
	cout << "Sistama equilibrato per 2000 simulazioni;" <<endl; 
...
void Averages( int iblk ) { //Print results for current block
	...
	cout << "Rateo di accetazione= " << accepted/attempted << endl << endl;
  	...
  	if ( iblk == 1 ) {
		Epot.open( "output.epot." + state + ".txt" );
		Pres.open( "output.pres." + state + ".txt" );
	} else {
		Epot.open( "output.epot." + state + ".txt", ios::app );
		Pres.open( "output.pres." + state + ".txt", ios::app );
	}
