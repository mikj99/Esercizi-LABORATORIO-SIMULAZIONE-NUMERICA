...
//simulazione
int nstep, L, seed;						//L:= grandezza del blocco per le medie a blocci; voglio sempre 100 blocchi
double delta;
bool first;
string state;
...
void ConfFinal();
void ConfXYZ(int);
void Measure( vector<double> &, vector<double> &, vector<double> &, vector<double> & );
...
int main() { 
	...
	cout << "\nCome funziona il programma di simulazione sull'Argon:" <<endl;
	cout << "1) essere in possesso di una buona configurazione precedente di partenza e nello stato effettivo della simulazione attraverso i file old.0 e final.0 (nel caso crearla);" <<endl;
	cout << "2) eseguire la simulazione (nel caso rieseguendo il programma);" <<endl <<endl;
	cout << "NOTA: qualora fosse necessario costruire una configurazione adeguata di partenza assicurarsi di scegliere la medesima fase per entrambe le simulazioni" <<endl <<endl;
	cout << "Fase da simulare? [solid/liquid/gas]" <<endl;
	cin >> state;
	...
	//int nconf = 1;
	for ( int i=1; i<= nstep/L; i++ ) {								// nstep/L = #blocchi	
		vector<double> Ek, Ep, Etot, T;
		for ( int k=1; k<=L; k++ ) {
			Move();
			if( k%10 == 0 ) {
				Measure( Ek, Ep, Etot, T );
				//ConfXYZ(nconf);
				//nconf++;
			}
		}	
		out <<fixed <<setprecision(6) << Media(Ek) << "\t" << Media(Ep) << "\t" << Media(Etot) << "\t" << Media(T) <<endl;
			
		if ( i%int(double(nstep/L)/100.) == 0 )
			cout <<double(i)/double(nstep/L)*100 << "%\t" <<flush;
		if ( i%int(double(nstep/L)/10.) == 0 )
			cout <<endl;	
	}
	cout <<endl;
...
void Measure( vector<double> &Ek, vector<double> &Ep, vector<double> &Etot, vector<double> &T ){
	int bin;
	double v, t, vij;
	double dx, dy, dz, dr;
...
//Kinetic energy
	...
	Ep.push_back(stima_pot);
	Ek.push_back(stima_kin);
	T.push_back(stima_temp);
